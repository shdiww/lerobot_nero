...
from .configuration_pico_vr import NeroPicoVRConfig
from .pico_vr_utils import PicoVRController
...
class NeroPicoVR(Teleoperator):
    config_class = NeroPicoVRConfig
    name = "pico_vr"

    def __init__(self, config: NeroPicoVRConfig):
        ...
        self._prev_grip_active = False
        self._grip_ref_ee_pos = None
        self._grip_ref_ee_rot = None
        self._grip_ref_ctrl_rot = None
    ...
    def connect(self, calibrate: bool = True) -> None:
        self.controller = PicoVRController(self.config)
    ...
    def get_action(self) -> RobotAction:
        ...
        grip_active = self.controller.is_active
        if grip_active and not self._prev_grip_active:
            logger.info("Grip activated — capturing EE + controller reference")
            self._grip_ref_ee_pos = self._ee_pos.copy()
            self._grip_ref_ee_rot = self._ee_rot.copy()
            self._grip_ref_ctrl_rot = self.controller.get_abs_rot_matrix().copy()
        if not grip_active:
            self._grip_ref_ee_pos = None
            self._grip_ref_ee_rot = None
            self._grip_ref_ctrl_rot = None
        self._prev_grip_active = grip_active
    ...
    def clear_events(self) -> None:
        if self.controller is not None:
            self.controller.clear_all_events()
    ...
    def solve_ik_from_deltas(
        ...
        if self._grip_ref_ee_pos is not None:
            ref_pos = self._grip_ref_ee_pos
            ref_rot = self._grip_ref_ee_rot
        else:
            ref_pos = self._ee_pos
            ref_rot = self._ee_rot
        ...
        if self._grip_ref_ctrl_rot is not None and self.controller.is_active:
            ctrl_rot = self.controller.get_abs_rot_matrix()
            offset = self._grip_ref_ee_rot @ self._grip_ref_ctrl_rot.T
            new_rot = offset @ ctrl_rot
        else:
            r_ref = Rotation.from_matrix(ref_rot)
            delta_r = Rotation.from_rotvec([delta_wx, delta_wy, delta_wz])
            new_rot = (delta_r * r_ref).as_matrix()
